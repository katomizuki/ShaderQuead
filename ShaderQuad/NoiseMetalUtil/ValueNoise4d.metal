...
// 疑似乱数生成
float pseudoRandom(float4 v)
{
    return -1.0 + 2.0 * fract(sin(dot(v, float4(732.1, 845.3, 231.7, 641.1))) * 43758.5453123);
}
...
float valueNoise(float4 x)
{
    ...
    // 4次元格子の補間
    float4 n_0w   = mix(float4(n0000, n1000, n0100, n1100), float4(n0001, n1001, n0101, n1101), u.w);
    float4 n_1w   = mix(float4(n0010, n1010, n0110, n1110), float4(n0011, n1011, n0111, n1111), u.w);
    float4 n_zw   = mix(n_0w, n_1w, u.z);
    float2 n_yzw  = mix(n_zw.xy, n_zw.zw, u.y);
    float  n_xyzw = mix(n_yzw.x, n_yzw.y, u.x);
